...
X_train_path = (data_splits_dir / "X_train_cell_injury_fs.csv.gz").resolve(strict=True)
X_test_path = (data_splits_dir / "X_test_cell_injury_fs.csv.gz").resolve(strict=True)
y_train_path = (data_splits_dir / "y_train_cell_injury_fs.csv.gz").resolve(strict=True)
y_test_path = (data_splits_dir / "y_test_cell_injury_fs.csv.gz").resolve(strict=True)

# shared feature space path
feature_space_path = (feature_dir / "fs_cell_injury_only_feature_space.json").resolve(
    strict=True
)

# holdout paths
plate_holdout_path = (data_splits_dir / "fs_plate_holdout.csv.gz").resolve(strict=True)
treatment_holdout_path = (data_splits_dir / "fs_treatment_holdout.csv.gz").resolve(
    strict=True
)
wells_holdout_path = (data_splits_dir / "fs_wells_holdout.csv.gz").resolve(strict=True)
...
# saving model paths
model_path = modeling_dir / "fs_multi_class_model.joblib"
shuffled_model_path = modeling_dir / "fs_shuffled_multi_class_model.joblib"

# file paths to save cv scors
original_model_cv_results = modeling_dir / "fs_multi_class_cv_results.csv"
shuffled_model_cv_results = modeling_dir / "fs_shuffled_multi_class_cv_results.csv"
...
# save scores
sorted_scores.to_csv(modeling_dir / "fs_coeff_scores_per_injury.csv", index=False)
