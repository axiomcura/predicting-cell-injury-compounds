...
X_train_path = (data_splits_dir / "X_train_aligned.csv.gz").resolve(strict=True)
X_test_path = (data_splits_dir / "X_test_aligned.csv.gz").resolve(strict=True)
y_train_path = (data_splits_dir / "y_train_aligned.csv.gz").resolve(strict=True)
y_test_path = (data_splits_dir / "y_test_aligned.csv.gz").resolve(strict=True)
...
# holdout paths
plate_holdout_path = (data_splits_dir / "aligned_plate_holdout.csv.gz").resolve(
    strict=True
)
treatment_holdout_path = (data_splits_dir / "aligned_treatment_holdout.csv.gz").resolve(
    strict=True
)
wells_holdout_path = (data_splits_dir / "aligned_wells_holdout.csv.gz").resolve(
    strict=True
)
...
# saving model paths
model_path = modeling_dir / "aligned_multi_class_model.joblib"
shuffled_model_path = modeling_dir / "aligned_shuffled_multi_class_model.joblib"

# file paths to save cv scors
original_model_cv_results = modeling_dir / "aligned_multi_class_cv_results.csv"
shuffled_model_cv_results = modeling_dir / "aligned_shuffled_multi_class_cv_results.csv"
...
all_f1_scores.to_csv(
    modeling_dir / "aligned_all_f1_scores.csv.gz", index=False, compression="gzip"
)
...
all_pr_scores.to_csv(
    modeling_dir / "aligned_precision_recall_scores.csv.gz",
    index=False,
    compression="gzip",
)
...
# saving pr scores
all_cm_dfs.to_csv(
    modeling_dir / "aligned_confusion_matrix.csv.gz", index=False, compression="gzip"
)
...
# save scores
sorted_scores.to_csv(modeling_dir / "aligned_coeff_scores_per_injury.csv", index=False)
