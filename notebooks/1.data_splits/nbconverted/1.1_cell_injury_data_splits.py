...
# # Splitting feature selected cell injury profiles
#
# Here, we utilize the feature-selected cell-injury profiles generated in the preceding module notebook [here](../0.feature_selection/0.feature_selection.ipynb), focusing on dividing the data into training, testing, and holdout sets for machine learning training.
#
...
# loading feature selected cell injury dataset
fs_profile_path = (fs_dir / "fs_cell_injury_only.csv.gz").resolve(strict=True)

# load in feature select cell injury feature space file
fs_feature_space = load_json_file(
    fpath=(fs_dir / "fs_cell_injury_only_feature_space.json").resolve(strict=True)
)
...
with open(data_split_dir / "fs_injury_metadata.json", mode="w") as stream:
...
# save dictionary into a json file
with open(data_split_dir / "fs_cell_injury_plate_info.json", mode="w") as stream:
...
wells_heldout_df.to_csv(
    data_split_dir / "fs_wells_holdout.csv.gz", index=False, compression="gzip"
)
...
X_train.to_csv(
    data_split_dir / "X_train_cell_injury_fs.csv.gz", compression="gzip", index=False
)
X_test.to_csv(
    data_split_dir / "X_test_cell_injury_fs.csv.gz", compression="gzip", index=False
)
y_train.to_csv(
    data_split_dir / "y_train_cell_injury_fs.csv.gz", compression="gzip", index=False
)
y_test.to_csv(
    data_split_dir / "y_test_cell_injury_fs.csv.gz", compression="gzip", index=False
)
...
cell_injury_metadata.to_csv(
    data_split_dir / "fs_cell_injury_metadata_after_holdout.csv.gz",
    compression="gzip",
    index=False,
)
# display
...
merged_summary_df.to_csv(
    data_split_dir / "cell_injury_fs_summary_data_split.csv", index=False
)
